...
public class FahrzeugService {
    ...
    public String fahrzeugAktu(Fahrzeug fahrzeug, String kfz) { // bu yeni eklendi
        Optional<Fahrzeug> opt = fahrzeugRepo.findFahrzeugByKfz(kfz);
        if (opt.isPresent()){
            Fahrzeug f = opt.get();
            f.setZustand("fertig");
            f.setReparaturen(fahrzeug.getReparaturen());
            f.setKosten(fahrzeug.getKosten());
            fahrzeugRepo.save(f);
            return "Fahrzeug ist erfolgreich aktualisiert.";
        }
        else{
            throw new NoSuchElementException("Fahrzeug objesi bulunamadı.");
        }
